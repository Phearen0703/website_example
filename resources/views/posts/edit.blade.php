            ...
            <div class="col-md-12 col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body pt-0">
                        <form class="form" action="{{ route('post.update',['id' => $row->id]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="mb-2">
                                ...
                                <select class="form-select" name="menu_id" required>
                                    ...
                                    @foreach($menus as $menu)
                                        <option value="{{ $menu->id }}" {{$row->menu_id == $menu->id ? 'selected' : ''}}>{{ $menu->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-2">
                                ...
                                <textarea class="form-control" name="contents" cols="30" rows="2">{{$row->content}}</textarea>
                            </div>
                            <div class="mb-2">
